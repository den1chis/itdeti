...
def _occurrences(slot: StudentScheduleSlot, start: date, end: date) -> int:
    ...
    first = start + timedelta(
        days=(slot.day_of_week - start.weekday()) % 7
    )
...
async def create_payment(
    ...
    if payload.request_id:
        existing = await db.scalar(
            select(Payment).where(
                Payment.request_id == payload.request_id
            )
        )
        if existing:
            return existing
    ...
    if payload.kaspi_notification_id:
        notification = await db.scalar(
            select(Notification).where(
                Notification.id == payload.kaspi_notification_id
            )
        )
        if not notification:
            raise HTTPException(
                404,
                "Kaspi notification not found",
            )

        existing_kaspi = await db.scalar(
            select(Payment).where(
                Payment.kaspi_notification_id == payload.kaspi_notification_id
            )
        )
        if existing_kaspi:
    ...
    db.add(payment)
    student.balance = (
        Decimal(student.balance) + Decimal(payload.amount)
    )
    ...
    try:
        ...
        if payload.kaspi_notification_id:
            existing = await db.scalar(
                select(Payment).where(
                    Payment.kaspi_notification_id
                    == payload.kaspi_notification_id
                )
            )
            if existing:
                return existing
...
async def list_payments(
    ...
    if from_date:
        query = query.where(
            Payment.recorded_at >= _local_start(from_date)
        )

    if to_date:
        query = query.where(
            Payment.recorded_at < _local_start(
...
async def finance_summary(
    ...
    income = await _sum_payments(
        db,
        _local_start(month_start),
        _local_start(next_month),
    )

    expense_total = await _sum_expenses(
        db,
        month_start,
        month_end,
    )
    ...
    by_category = {
        ...
        for category, amount in categories_result.all()
    }

    all_income = await db.scalar(
        select(
            func.coalesce(
                func.sum(Payment.amount),
                0,
    ...
    total_balance = (
        Decimal(all_income or 0)
        - Decimal(all_expenses or 0)
    )

    scheduled_count, scheduled_income = await _schedule_counts(
        db,
        month_start,
        month_end,
    )
    ...
    _, forecast_income = await _schedule_counts(
        db,
        forecast_start,
        month_end,
    )
