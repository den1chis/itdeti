...
async def _parents_for_student(db: AsyncSession, student_id: UUID) -> list[Parent]:
    result = await db.execute(select(Parent).join(StudentParent, StudentParent.parent_id == Parent.id).where(StudentParent.student_id == student_id).order_by(Parent.full_name))
...
async def _student_response(db: AsyncSession, student: Student) -> StudentResponse:
    ...
    return StudentResponse(id=student.id, user_id=student.user_id, full_name=student.full_name, birth_date=student.birth_date, course=student.course, notes=student.notes, balance=student.balance, lesson_price=student.lesson_price, is_active=student.is_active, created_at=student.created_at, updated_at=student.updated_at, remaining_lessons=remaining, current_month_lessons=current_lessons, current_month_due=current_due, parents=await _parents_for_student(db, student.id))
...
async def create_parent(payload: ParentCreate, db: AsyncSession = Depends(get_db), _: User = Depends(teacher_or_admin)):
    parent = Parent(**payload.model_dump()); db.add(parent); await db.commit(); await db.refresh(parent); return parent
...
async def list_parents(db: AsyncSession = Depends(get_db), _: User = Depends(get_current_user)):
    result = await db.execute(select(Parent).order_by(Parent.full_name)); return result.scalars().all()
...
async def update_parent(parent_id: UUID, payload: ParentUpdate, db: AsyncSession = Depends(get_db), _: User = Depends(teacher_or_admin)):
    ...
    if not parent: raise HTTPException(404, "Parent not found")
    for field, value in payload.model_dump(exclude_none=True).items(): setattr(parent, field, value)
    await db.commit(); await db.refresh(parent); return parent
...
async def create_student(payload: StudentCreate, db: AsyncSession = Depends(get_db), _: User = Depends(teacher_or_admin)):
    if payload.balance < 0: raise HTTPException(400, "Initial balance cannot be negative")
    student = Student(**payload.model_dump(exclude={"parent_ids"})); db.add(student); await db.flush()
    for parent_id in payload.parent_ids:
        if not await db.scalar(select(Parent.id).where(Parent.id == parent_id)): raise HTTPException(404, f"Parent {parent_id} not found")
        db.add(StudentParent(student_id=student.id, parent_id=parent_id))
    await db.commit(); await db.refresh(student); return await _student_response(db, student)
...
async def list_students(active_only: bool = True, db: AsyncSession = Depends(get_db), _: User = Depends(get_current_user)):
    ...
    if active_only: query = query.where(Student.is_active.is_(True))
    result = await db.execute(query); return [await _student_response(db, student) for student in result.scalars().all()]
...
async def update_student(student_id: UUID, payload: StudentUpdate, db: AsyncSession = Depends(get_db), _: User = Depends(teacher_or_admin)):
    ...
    if "balance" in changes and Decimal(changes["balance"]) < 0: raise HTTPException(400, "Balance cannot be negative")
    for field, value in changes.items(): setattr(student, field, value)
    await db.commit(); await db.refresh(student); return await _student_response(db, student)


@router.delete("/students/{student_id}", status_code=204)
async def deactivate_student(student_id: UUID, db: AsyncSession = Depends(get_db), _: User = Depends(teacher_or_admin)):
    """Deactivate student and remove active recurring schedule plus future generated lessons atomically.
    Completed/past lessons and financial history are preserved.
    """
    student = await db.scalar(select(Student).where(Student.id == student_id).with_for_update())
    if not student: raise HTTPException(404, "Student not found")
    now = datetime.now().astimezone()
    result = await db.execute(select(Lesson, Event).join(Event, Event.id == Lesson.event_id).where(Lesson.student_id == student_id, Lesson.status.in_(["scheduled", "rescheduled"]), Event.start_time >= now))
    for lesson, event in result.all():
        await db.delete(lesson); await db.delete(event)
    await db.execute(StudentScheduleSlot.__table__.update().where(StudentScheduleSlot.student_id == student_id).values(is_active=False, valid_until=date.today()))
...
async def link_parent(student_id: UUID, parent_id: UUID, db: AsyncSession = Depends(get_db), _: User = Depends(teacher_or_admin)):
    if not await db.scalar(select(Student.id).where(Student.id == student_id)): raise HTTPException(404, "Student not found")
    if not await db.scalar(select(Parent.id).where(Parent.id == parent_id)): raise HTTPException(404, "Parent not found")
    if await db.scalar(select(StudentParent).where(StudentParent.student_id == student_id, StudentParent.parent_id == parent_id)): raise HTTPException(409, "Parent already linked")
    db.add(StudentParent(student_id=student_id, parent_id=parent_id)); await db.commit(); return {"detail": "Parent linked"}
...
async def unlink_parent(student_id: UUID, parent_id: UUID, db: AsyncSession = Depends(get_db), _: User = Depends(teacher_or_admin)):
    await db.execute(delete(StudentParent).where(StudentParent.student_id == student_id, StudentParent.parent_id == parent_id)); await db.commit()
...
async def create_schedule_slot(student_id: UUID, payload: ScheduleSlotCreate, db: AsyncSession = Depends(get_db), _: User = Depends(teacher_or_admin)):
    if not await db.scalar(select(Student.id).where(Student.id == student_id, Student.is_active.is_(True))): raise HTTPException(404, "Active student not found")
    duplicate = await db.scalar(select(StudentScheduleSlot).where(StudentScheduleSlot.student_id == student_id, StudentScheduleSlot.day_of_week == payload.day_of_week, StudentScheduleSlot.start_time == payload.start_time, StudentScheduleSlot.valid_from == payload.valid_from))
    if duplicate:
        if not duplicate.is_active:
            for field, value in payload.model_dump().items(): setattr(duplicate, field, value)
            duplicate.is_active = True; duplicate.valid_until = None
            await db.commit(); await db.refresh(duplicate); return duplicate
        raise HTTPException(409, "This schedule entry already exists")
    slot = StudentScheduleSlot(student_id=student_id, **payload.model_dump()); db.add(slot)
    try:
        await db.commit()
    except IntegrityError as exc:
        await db.rollback(); raise HTTPException(409, "This schedule entry already exists") from exc
    await db.refresh(slot); return slot
...
async def list_schedule_slots(student_id: UUID, include_inactive: bool = False, db: AsyncSession = Depends(get_db), _: User = Depends(get_current_user)):
    ...
    if not include_inactive: query = query.where(StudentScheduleSlot.is_active.is_(True))
    result = await db.execute(query.order_by(StudentScheduleSlot.day_of_week, StudentScheduleSlot.start_time)); return result.scalars().all()
...
async def update_schedule_slot(student_id: UUID, slot_id: UUID, payload: ScheduleSlotUpdate, db: AsyncSession = Depends(get_db), _: User = Depends(teacher_or_admin)):
    ...
    if not slot: raise HTTPException(404, "Schedule entry not found")
    changes = payload.model_dump(exclude_none=True)
    new_day = changes.get("day_of_week", slot.day_of_week); new_time = changes.get("start_time", slot.start_time); new_from = changes.get("valid_from", slot.valid_from); new_until = changes.get("valid_until", slot.valid_until)
    if new_until and new_until < new_from: raise HTTPException(400, "valid_until must be on or after valid_from")
    ...
    if duplicate: raise HTTPException(409, "This schedule entry already exists")
    future = await db.execute(select(Lesson, Event).join(Event, Event.id == Lesson.event_id).where(Lesson.schedule_slot_id == slot.id, Lesson.status.in_(["scheduled", "rescheduled"]), Lesson.original_start_time.is_not(None), Lesson.original_start_time >= datetime.now().astimezone()))
    for lesson, event in future.all(): await db.delete(lesson); await db.delete(event)
    for field, value in changes.items(): setattr(slot, field, value)
    try:
        await db.commit()
    except IntegrityError as exc:
        await db.rollback(); raise HTTPException(409, "This schedule entry already exists") from exc
...
async def deactivate_schedule_slot(student_id: UUID, slot_id: UUID, db: AsyncSession = Depends(get_db), _: User = Depends(teacher_or_admin)):
    ...
    future = await db.execute(select(Lesson, Event).join(Event, Event.id == Lesson.event_id).where(Lesson.schedule_slot_id == slot.id, Lesson.status.in_(["scheduled", "rescheduled"]), Lesson.original_start_time.is_not(None), Lesson.original_start_time >= datetime.now().astimezone()))
    for lesson, event in future.all(): await db.delete(lesson); await db.delete(event)
    slot.is_active = False; slot.valid_until = date.today(); await db.commit()
...
async def student_balance(student_id: UUID, db: AsyncSession = Depends(get_db), _: User = Depends(get_current_user)):
    student = await db.scalar(select(Student).where(Student.id == student_id))
    if not student: raise HTTPException(404, "Student not found")
    now = date.today(); month_start = date(now.year, now.month, 1); month_end = date(now.year, now.month, monthrange(now.year, now.month)[1])
    ...
    return BalanceResponse(student_id=student.id, balance=student.balance, remaining_lessons=int(max(Decimal("0"), Decimal(student.balance)) // Decimal(student.lesson_price)), current_month_lessons=month_lessons, current_month_due=month_due, payments_this_month=payments_total, monthly_debt=max(Decimal("0"), month_due - payments_total))